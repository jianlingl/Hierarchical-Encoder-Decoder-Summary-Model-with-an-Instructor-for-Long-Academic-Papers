...
tf.app.flags.DEFINE_string(
    'data_path', './data/small.bin', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string(
    'extra_path', './data/small.bin', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string(
    'vocab_path', './data/vocab', 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_boolean('Increase_section_semantics', False, 'use section_semantics model')
tf.app.flags.DEFINE_boolean('Increase_doc_semantics', True, 'use doc_semantics model')

tf.app.flags.DEFINE_boolean('single_pass', False, 'For decode mode only. If True, run eval on the full dataset using a fixed checkpoint, i.e.'
                                                  'take the current checkpoint, and use it to produce one summary for each example in the dataset, '
                                                  'write the summaries to file and then get ROUGE scores for the whole dataset. If False (default), '
                                                  'run concurrent decoding, i.e. repeatedly load latest checkpoint, use it to produce summaries for'
...
tf.app.flags.DEFINE_string(
    'exp_name', 'test-experiment', 'Name for experiment. Logs will be saved in a directory with this name, under log_root.')
...
tf.app.flags.DEFINE_integer(
    'max_enc_steps', 2500, 'max timesteps of encoder (max source text tokens)')
tf.app.flags.DEFINE_integer(
    'max_dec_steps', 210, 'max timesteps of decoder (max summary tokens)')
tf.app.flags.DEFINE_integer(
...
tf.app.flags.DEFINE_integer(
    'vocab_size', 50000, 'Size of vocabulary. These will be read from the vocabulary file in order. '
...
tf.app.flags.DEFINE_integer('max_section_len', 500, 'Truncate sections')
...
tf.app.flags.DEFINE_integer('num_sections', 5, 'Maximum introduction section length')
tf.app.flags.DEFINE_integer('max_extract_len', 210, 'Maximum input extract length')


tf.app.flags.DEFINE_boolean('hier', True, 'Hierarchical model to utilize section information')
tf.app.flags.DEFINE_boolean('phased_lstm', False, 'Hierarchical model to utilize section information')
tf.app.flags.DEFINE_boolean('output_weight_sharing', False, 'If True, the weights of the model are '
                                                            'shared between embedding and output projection layer')
tf.app.flags.DEFINE_boolean('use_do', True, 'If True, use drop out on lstm cells in the encoder')
tf.app.flags.DEFINE_float('do_prob', 0.25, 'Dropout probability in lstm cells')

tf.app.flags.DEFINE_boolean('pretrained_embeddings', False, 'use pretrained embeddings')
tf.app.flags.DEFINE_string('embeddings_path', 'emb.txt', 'path to plain text embedding files')
...
tf.app.flags.DEFINE_boolean('coverage', False, 'Use coverage mechanism. Note, the experiments reported in the ACL paper '
                                               ...
                                               'WITH coverage afterwards. i.e. to reproduce the results in the ACL paper, '
...
tf.app.flags.DEFINE_boolean('split_intro', True, 'Split intro into first and last parts.')
tf.app.flags.DEFINE_float('temperature', 0.0, 'simulating temperature for softmax on attn_words')

tf.app.flags.DEFINE_boolean('legacy_encoder', False, 'Use the older encoder (for compatibility issues).')
tf.app.flags.DEFINE_boolean('fixed_attn', True, 'Use the older decoder (for compatibility issues).')
...
def main(unused_argv):
    ...
    hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm',
                   ...
                   'num_sections', 'hier', 'phased_lstm', 'output_weight_sharing', 'use_do' ,'do_prob', 'embeddings_path',
                   'pretrained_embeddings', 'pubmed', 'num_gpus', 'split_intro', 'temperature','vocab_size','max_extract_len',
                   'Increase_section_semantics','Increase_doc_semantics']
    hps_dict = {}
    ...
    if hps.mode == 'train':
        ...
        # This will be the hyperparameters for the decoder model  这将是解码器模型的超参数
        # The model is configured with max_dec_steps=1 because we only ever run one step of the decoder at a time (to do beam search).
        # 该模型配置为max_dec_steps = 1，因为我们一次只能运行解码器的一个步骤（进行波束搜索）。
        # Note that the batcher is initialized with max_dec_steps equal to e.g. 100
        #请注意，该批处理程序的max_dec_steps等于例如100
