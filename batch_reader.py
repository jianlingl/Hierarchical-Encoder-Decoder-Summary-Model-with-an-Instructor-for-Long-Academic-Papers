...
def extra_string_to_list(extract, dtype='str'):
  ...
  """
  extra = extract.replace('<s>', '')
  if len(extra.split())<450:
     print("here the extra is less than 500: ", str(len(extra.split())))
...
def _section_to_ids(section, vocab, max_len, pad_id):
  """ Converts words in a section (list of strings) to ids
...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences,extract_sum_sentence, article_id, sections, section_names, labels, vocab, hps):
...
#           self.enc_sec_len = sec_len # TODO: Check
        ...
        # ------>  _extract_sum ----- id
        extract_sum=' '.join(extract_sum_sentence)
        extract_sum__words=extract_sum.split()
        self.extract_len = min(len(extract_sum__words), self.hps.max_extract_len)
        extract_sum__words=_pad_words(extract_sum__words, self.hps.max_extract_len, data.PAD_TOKEN)
        self.extract_sum_id=[vocab.word2id(w) for w in extract_sum__words]
        ...
        # self._extract_sum=self.get_extractsum_seqs(abs_ids, hps.max_extract_len, start_decoding, stop_decoding)
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        self.extract_padding_mask = np.zeros((hps.batch_size, hps.max_extract_len), dtype=np.float32)
        self._extract_sum = np.zeros((hps.batch_size, hps.max_extract_len), dtype=np.int32)
        ...
        for i, ex in enumerate(example_list):
            ...
            self._extract_sum[i, :] = ex.extract_sum_id[:]

            for j in range(ex.extract_len):
                self.extract_padding_mask[i][j] = 1
...
#             ex.pad_section_input(max_num_sections, self.sec_pad_id)
    ...
    def init_decoder_seq(self, example_list, hps):
        ...
        # for i in range(len(self.dec_batch)):
        #     for j in range(len(self.dec_batch[i])):
        #         if self.dec_batch[i][j]>= hps.vocab_size:
        #             self.dec_batch[i][j]=1
...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, hps, extra_path, single_pass,
                 article_id_key,
        ...
        self.id_extract_dict = self.get_extract()
        ...
        if single_pass:
            ...
            self._num_batch_q_threads = int(self._num_example_q_threads / self._hps.batch_size)  # num threads to fill batch queue
    ...
    def get_extract(self):
        extract_dict = {}
        reader = open(self._extra_path, 'rb')
        while True:
            ...
            try:
                extract_dict[self._get_example_feature(example, 'article_id')] = extra_string_to_list(
                    self._get_example_feature(example, 'extract'))
            except ValueError:
                tf.logging.error(
    ...
    def _fill_example_queue(self):
        """Reads data from file and processes into Examples which are
        then placed into the example queue."""
        ...
        while True:
            try:
                ...
                # both strings.
                (article_id, article_text, abstract_sents, labels,section_names, sections) = six.next(input_gen)
...
#       abstract_sentences = [sent.strip() for sent in data.abstract2sents(''.join(abstract_sents))]
            ...
            if abst_len > self._hps.max_abstract_len or\
                ...
                continue
            extract_sum_sentence = self.id_extract_dict[article_id]
            # Process into an Example.
            example = Example(article_text, abstract_sentences,extract_sum_sentence, article_id, sections, section_names, labels,
