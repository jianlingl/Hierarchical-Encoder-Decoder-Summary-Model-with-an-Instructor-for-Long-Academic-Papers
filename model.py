...
class SummarizationModel(object):
  ...
  def __init__(self, hps, vocab, num_gpus):
    ...
    if FLAGS.new_attention and FLAGS.hier:
      ...
      tf.logging.info("       if FLAGS.new_attention and FLAGS.hier   ")
  ...
  def _add_placeholders(self):
    ...
    self._extract_padding_mask = tf.placeholder(tf.float32, [hps.batch_size, None], name='extract_padding_mask')
    if self._hps.hier:
    ...
    self._extract_sum = tf.placeholder(tf.int32, [hps.batch_size, hps.max_extract_len], name='extract_sum')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    if self._hps.hier:
      ...
      feed_dict[self._extract_padding_mask] = batch.extract_padding_mask
    else:
    ...
    feed_dict[self._extract_sum] = batch._extract_sum
  ...
  # 抽取出摘要的rnn encode
  def _extract_enc(self):
    hps = self._hps
    vsize = self._vocab.size()  # size of the vocabulary
    with tf.variable_scope('extract_enc'):
      self.rand_unif_init = tf.random_uniform_initializer(-hps.rand_unif_init_mag, hps.rand_unif_init_mag, seed=123)
      self.trunc_norm_init = tf.truncated_normal_initializer(stddev=hps.trunc_norm_init_std)
      with tf.variable_scope('extract_embeddings'):
        if hps.pretrained_embeddings:
          ...
          self.embedding = tf.get_variable('embedding', [vsize, hps.emb_dim],
                                           dtype=tf.float32, initializer=tf.constant_initializer(self.word2vec))
        ...
        emb_enc_inputs = tf.nn.embedding_lookup(self.embedding, self._extract_sum)

        z_shape = tf.shape(self._extract_sum)
        sess = tf.Session()
        tf.logging.info("\n\n ********************self._dec_batch ********************")
        tf.logging.info(sess.run(z_shape))

        target_batchs = tf.reshape(self._extract_sum, shape=[hps.batch_size, 1, hps.max_extract_len])

        z_shape = tf.shape(target_batchs)
        sess = tf.Session()
        tf.logging.info("\n\n ********************self._target_batch 2********************")
        tf.logging.info(sess.run(z_shape))

        if self._hps.hier:
          enc_batch_sections = tf.unstack(target_batchs, axis=1)
        ...
        z_shape = tf.shape(sec_emb_enc_inputs)
        sess = tf.Session()
        tf.logging.info("\n\n ********************sec_emb_enc_inputs ********************")
        tf.logging.info(sess.run(z_shape))
        ...
        if self._hps.hier:
          ...
          with tf.variable_scope("word_encoder", reuse=tf.AUTO_REUSE) as scope:
            ...
            tf.logging.info("\n\n ********************_extract_enc  end ********************")
            return states,encoder_outputs_words

  # def _combine_extracts_sections_hstate(self, extract_state, section_state):
  #   """
  #   using a linear layer to combine the extracted sum and the section
  #   :param extract_state:
  #   :param section_state:
  #   :return: tf.contrib.rnn.LSTMStateTuple(section_state.c, new_hidden) new_hidden = w1 * extract_state.h + W2 * section_state.h +b
  #   """
  #   hidden_dim = self._hps.hidden_dim
  #   w_extract_state_h = tf.get_variable( [hidden_dim, hidden_dim], dtype=tf.float32,
  #                                       initializer=self.trunc_norm_init)
  #   w_section_state_h = tf.get_variable( [hidden_dim, hidden_dim], dtype=tf.float32,
  #                                       initializer=self.trunc_norm_init)
  #   bias_reduce_h = tf.get_variable([hidden_dim], dtype=tf.float32,
  #                                   initializer=self.trunc_norm_init)
  #
  #   extract_state_h = extract_state.h
  #   section_state_h = section_state.h
  #   new_hidden_h = tf.nn.relu(
  #     tf.matmul(extract_state_h, w_extract_state_h) + tf.matmul(section_state_h, w_section_state_h) + bias_reduce_h)
  #
  #
  #   w_extract_state_c = tf.get_variable( [hidden_dim, hidden_dim], dtype=tf.float32,
  #                                       initializer=self.trunc_norm_init)
  #   w_section_state_c = tf.get_variable( [hidden_dim, hidden_dim], dtype=tf.float32,
  #                                       initializer=self.trunc_norm_init)
  #   bias_reduce_c = tf.get_variable( [hidden_dim], dtype=tf.float32,
  #                                   initializer=self.trunc_norm_init)
  #
  #   extract_state_c = extract_state.c
  #   section_state_c = section_state.c
  #   new_hidden_c = tf.nn.relu(
  #     tf.matmul(extract_state_c, w_extract_state_c) + tf.matmul(section_state_c, w_section_state_c) + bias_reduce_c)
  #
  #   return tf.contrib.rnn.LSTMStateTuple(new_hidden_c, new_hidden_h)


  def _add_seq2seq(self):
    """Add the whole sequence-to-sequence model to the graph."""
    abstract_enc_state=None

    if FLAGS.pretrained_embeddings:
      self.word2vec = load_embeddings(FLAGS.embeddings_path, self._vocab.word2id, FLAGS.rand_unif_init_mag)

    if self._hps.Increase_doc_semantics or self._hps.Increase_section_semantics:
      abstract_enc_state,  en_extract_state  = self._extract_enc()
      
    denselist = []

    z_shape = tf.shape(self._dec_batch)
    sess = tf.Session()
    tf.logging.info("\n\n ********************self._dec_batch ********************")
    tf.logging.info(sess.run(z_shape))
    ...
    with tf.variable_scope('seq2seq'):
      ...
      if self._hps.hier:
        with tf.variable_scope('encoder'), tf.device(self._next_device()):
          ...
          with tf.variable_scope("word_level_encoder", reuse=tf.AUTO_REUSE) as scope:
            ...
            if self._hps.Increase_section_semantics:
              for i in range(1):
                densename='Dense'+str(i)
                with tf.variable_scope(densename, reuse=tf.AUTO_REUSE) as scope:
                  denselist.append(Connect_Layer(self._hps.hidden_dim, True, tf.nn.relu))
              for i in range(self._hps.num_sections):
                tf.logging.info(i)
                encoder_tmp_output, (fw_st, bw_st) = tf.nn.bidirectional_dynamic_rnn(
                  cell_fw, cell_bw, inputs=sec_emb_enc_inputs[i], dtype=tf.float32,
                  sequence_length=self._batch_sections_len[:, i], swap_memory=True,
                  initial_state_bw=bw_st, initial_state_fw=fw_st)
                # concatenate the forwards and backwards states ext_emb_enc_inputs
                encoder_tmp_output = tf.concat(axis=2,values=encoder_tmp_output)  # shape=[batch x seq_len x hidden_size]

                encoder_outputs_words.append(encoder_tmp_output)
                # instead of concating the fw and bw states, we use a ff network
                # extract_state = self._reduce_states(fw_st_ext, fw_st_ext)
                section_state = self._reduce_states(fw_st, bw_st)


                artical_sec_state_h = denselist[0](section_state.h, abstract_enc_state[0].h)
                artical_sec_state_c = denselist[0](section_state.c, abstract_enc_state[0].c)
                combined_state = tf.contrib.rnn.LSTMStateTuple(artical_sec_state_h, artical_sec_state_c)
          ...
          en_extract_state_att=en_extract_state
          if self._hps.Increase_doc_semantics or self._hps.Increase_section_semantics:
              en_extract_state_att= tf.stack(en_extract_state_att, axis=1)
              shapes = en_extract_state_att.shape
              en_extract_state_att = tf.reshape(en_extract_state_att, (shapes[0].value, -1, shapes[-1].value))  # shape=[batch x (seq_len * num_sections) x hidden_size]
      ...
      if self._hps.Increase_doc_semantics:
        dense1 = Connect_Layer(self._hps.hidden_dim, True, tf.nn.relu)
        dense2 = Connect_Layer(self._hps.hidden_dim, True, tf.nn.relu)
        artical_state_h = dense1(self._dec_in_state.h, abstract_enc_state[0].h)
        artical_state_c = dense2(self._dec_in_state.c, abstract_enc_state[0].c)
        combined_state = tf.contrib.rnn.LSTMStateTuple(artical_state_h, artical_state_c)
        self._dec_in_state = combined_state
      ...
      with tf.variable_scope('decoder'), tf.device(self._next_device()):
        ...
        if self._hps.hier:
          decoder_outputs, self._dec_out_state, self.attn_dists, self.attn_dist_extracts, self.p_gens, self.coverage, self.attn_dists_sec = \
            self.attn_decoder(emb_dec_inputs,  # decode的输入
                              self._dec_in_state,  # 上下文向量
                              cell,

                              self._enc_states,  # word级别的state
                              extract_state=en_extract_state_att,  # extract级别的state
        ...
        # else:
        #   decoder_outputs, self._dec_out_state, self.attn_dists, self.attn_dist_extracts, self.p_gens, self.coverage, _ = \
        ...
        #                       enc_padding_mask=self._enc_padding_mask,
        #                       # enc_padding_extract_mask=self._enc_padding_extract_mask,
        #                       initial_state_attention=(self._hps.mode == "decode"),
        #                       pointer_gen=self._hps.pointer_gen,
        #                       use_coverage=self._hps.coverage,
        #                       prev_coverage=prev_coverage,
  ...
  #         w_t = tf.transpose(w)
        ...
        vocab_scores = []
        tf.logging.info("           decoder_outputs length")
        tf.logging.info(len(decoder_outputs))
...
#       import pdb; pdb.set_trace()
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    feed = {
        ...
        self._extract_padding_mask: batch.extract_padding_mask,
    ...
    feed[self._extract_sum] = batch._extract_sum
