...
def load_embeddings0(fpath, word_to_id, emd_init_var=0.25):
...
def load_embeddings(fpath, word_to_id, emd_init_var=0.25):
    ...
    num_words, vector_size = list(map(int, next(f_iter).strip().split(' ')))
    np.random.seed(123)
    embd = np.random.uniform(-emd_init_var, emd_init_var, (FLAGS.vocab_size, vector_size))
    i = 0
    tf.logging.info('loading word embeddings')

    for line in f_iter:
        i += 1
        if i == 0:
            continue
        if i>FLAGS.vocab_size:
            break
        row = line.strip().split('##')
        word = row[0]
        vecstr = row[1].replace(' ', '').replace('[', '').replace(']', '')
        veclist = vecstr.split(',')
        vec = list(map(float, veclist))
        embd[word_to_id(word)] = np.array(vec)
    print('embeddings loaded, vocab size: {}'.format(num_words))
...
tf.logging.info(get_available_gpus())