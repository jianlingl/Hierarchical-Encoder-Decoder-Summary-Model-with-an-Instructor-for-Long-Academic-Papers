...
"""This file contains code to read the train/eval/test data from file 
and process it, and read the vocab data from file and process it"""
...
class Vocab(object):
  ...
  def __init__(self, vocab_file, max_size):
    ...
    # [SPAD], [UNK], [PAD], [START] and [STOP] get the ids 0,1,2,3.
    for w in [SEC_PAD_TOKEN, UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING]:
    ...
    with codecs.open(vocab_file, 'r', encoding='utf-8') as vocab_f:
      for line in vocab_f:
        ...
        if len(pieces) != 2:
          ...
          continue
        w = pieces[0]
        if w in [SENTENCE_START, SENTENCE_END, UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING, SEC_PAD_TOKEN]:
...
def example_generator(data_path, single_pass):
  ...
  Args:
    ...
    single_pass:
      Boolean. If True, go through the dataset exactly once, generating examples in the order they appear,
      then return. Otherwise, generate random examples indefinitely.
  Yields:
...
def show_art_oovs(article, vocab):
  ...
  words = [t for t in ' '.join(article).split(' ')]
